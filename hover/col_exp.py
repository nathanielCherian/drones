"""
col_exp.py: Training script for CollisionAviary.
PPO trains drone 0 to fly toward and collide with drone 1 (PID-controlled stationary drone).
"""
...
from CollisionAviary import CollisionAviary
...
class CollisionEvalCallback(BaseCallback):
    ...
    def evaluate_and_log(self):
        ...
        if len(frames) > 0:
            os.makedirs("eval_videos/collision_vel", exist_ok=True)
            video_path = f"eval_videos/collision_vel/collision_vel_step_{self.num_timesteps}.mp4"
...
DEFAULT_ACT = ActionType('vel')  # Use velocity control - PID handles motors


def run(from_model=None, total_timesteps=150000):
    """
    Train PPO on CollisionAviary.
    ...
    if from_model:
        model = PPO.load(from_model, env=sb3_env, verbose=1,
                         tensorboard_log="./ppo_collision_tensorboard/")
    ...
    model.learn(total_timesteps=total_timesteps, callback=eval_callback, tb_log_name="PPO_Collision")

    # Save final model
    os.makedirs("models", exist_ok=True)
    model_path = f"models/ppo_collision_{total_timesteps // 1000}k"
