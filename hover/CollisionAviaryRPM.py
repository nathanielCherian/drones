"""
CollisionAviaryRPM: Two-drone environment where PPO controls one drone
using raw RPM motor commands, and the other hovers using a PID controller.
Reward is based on distance to the stationary drone + bonus for collision.

This is the HARD version - PPO must learn both stabilization AND navigation.
"""
...
class CollisionAviaryRPM(BaseRLAviary):
    """Two-drone env: PPO controls drone 0 with raw RPMs, PID holds drone 1 stationary."""
    ...
    def __init__(self,
                 ...
                 act: ActionType = ActionType.RPM,  # Raw motor control
                 ):
        """
        Parameters
        ----------
        See HoverAviary for details. Key difference: 2 drones, RPM control.
        """
    ...
    def step(self, action):
        ...
        action: The PPO action for drone 0 only (raw RPM, shape (1, 4)).
        ...
        # Preprocess the PPO action for drone 0
        ppo_action = np.array(action).reshape(1, -1)

        # Build combined action array: PPO RPM for drone 0, PID RPM for drone 1
    ...
    def _computeReward(self):
        ...
        pos0, pos1 = state0[0:3], state1[0:3]
        vel0 = state0[10:13]
        rpy0 = state0[7:10]  # roll, pitch, yaw
        ang_vel0 = state0[13:16]
        
        # 1. Distance reward (exponential shaping is smoother)
        distance = np.linalg.norm(pos1 - pos0)
        dist_reward = np.exp(-distance)  # Ranges 0.18 (at 1.7m) to 1.0 (at contact)
        
        # 2. Approach velocity reward (reward moving toward target)
        direction = (pos1 - pos0) / (distance + 1e-6)
        approach_vel = np.dot(vel0, direction)
        vel_reward = 0.1 * np.clip(approach_vel, -1, 2)  # Reward approaching, penalize retreating
        
        # 3. Stability rewards/penalties
        roll_penalty = -0.5 * abs(rpy0[0])
        pitch_penalty = -0.5 * abs(rpy0[1])
        ang_vel_penalty = -0.05 * np.linalg.norm(ang_vel0)
        
        # 4. Altitude reward (encourage staying airborne)
        altitude_reward = 0.2 * np.clip(pos0[2], 0, 1.5)  # Up to 0.3 for being at target altitude
        
        # 5. Control smoothness (requires storing previous action)
        if hasattr(self, 'prev_action') and self.prev_action is not None:
            action_diff = np.linalg.norm(self.action_buffer[-1] - self.prev_action)
            control_penalty = -0.02 * action_diff
        else:
            control_penalty = 0.0
        ...
        # 7. Collision bonus (shaped by approach velocity for smoothness)
        ...
        if len(contacts) > 0:
            reward += 20.0 + 5.0 * max(0, approach_vel)  # Bonus for hitting while approaching fast
