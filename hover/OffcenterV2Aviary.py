"""
OffcenterV2Aviary: Stage 2 of curriculum - stationary sphere offset at [0.3, 0.3, 0.7].
...
class OffcenterV2Aviary(HoverAviary):

    def __init__(self, *args, **kwargs):
        super().__init__(**kwargs)
        self.EPISODE_LEN_SEC = 30
        self.TARGET_RADIUS = 0.05
        self.TARGET_POS = np.array([0.3, 0.3, 0.7])  # Offset position
    ...
    def _addTargetSphere(self):
        ...
        visual_shape = p.createVisualShape(
            shapeType=p.GEOM_SPHERE,
            radius=self.TARGET_RADIUS,
            rgbaColor=[0.8, 0.8, 0.2, 0.9],  # Yellow
            physicsClientId=self.CLIENT
        )
